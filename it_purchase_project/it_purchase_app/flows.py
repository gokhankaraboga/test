...
class PurchaseFlow(Flow):
    ...
    support = (
        flow.View(
            view.SupportView,
            task_description=_("Support Team"),
        )
    ...
    check_support_approve = (
        ...
        ).Then(this.price_quote).Else(this.end)
    )
    ...
    price_quote = (
        ...
        ).Permission('profile.can_purchase_team').Next(this.check_price_quote)
    )
    ...
    check_price_quote = (
        ...
        ).Then(this.get_price_quote).Else(this.is_superior_approval_required)
    )
